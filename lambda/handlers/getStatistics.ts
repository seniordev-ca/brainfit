...
export async function GetStatistics(userID, req) {
  /** @type {import("../layers/shared/nodejs/databaseHelper")} */
  const database = new DatabaseHelper();

  const queryResult = await database.getStats({
    userID,
    ...req
  });
...
export const handler = async (event, context, callback) => {
  try {
    ...
    const stats = await GetStatistics(user.uid, event.queryStringParameters);

    callback(null, lambdaResponse(200, stats));
  } catch (error) {
